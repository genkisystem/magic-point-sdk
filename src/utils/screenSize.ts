...
export const getCurrentScreenSizeRange = (
...
export const checkTaskScreenSizeToRender = (
    taskScreenSize: number,
    currentWindowScreenSize: WindowSizeRange = getCurrentScreenSizeRange(),
): boolean => {
    return (
        taskScreenSize >= currentWindowScreenSize.from &&
        taskScreenSize < currentWindowScreenSize.to
    );
};
...
export const defineMediaQueriesAndSetupEventListener = (
    ...
    for (let i = 0; i < sortedBreakpoints.length; i++) {
        if (i === 0) {
            mediaQueries.push({
                query: window.matchMedia(
                    `(max-width: ${sortedBreakpoints[i][1] - 1}px)`,
                ),
                label: i18next.t("screenSize:size.smallestScreen"),
                lowerBound: 0,
                upperBound: sortedBreakpoints[i][1] - 1,
            });
            continue;
        }
        ...
        if (i === sortedBreakpoints.length - 1) {
            mediaQueries.push({
                query: window.matchMedia(
                    `(min-width: ${sortedBreakpoints[i][1]}px)`,
                ),
                label: i18next.t("screenSize:size.largestSreen"),
                lowerBound: sortedBreakpoints[i][1],
        ...
        mediaQueries.push({
            ...
            lowerBound: sortedBreakpoints[i][1],
            upperBound: sortedBreakpoints[i + 1][1] - 1,
        });
    }

    mediaQueries.forEach((mq) => {
        mq.query.onchange = (e: MediaQueryListEvent) =>
            handleWindowSizeChange(e, mq.label, mq.lowerBound, mq.upperBound);
    });
};
