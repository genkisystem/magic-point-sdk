...
export class FormManager extends Base {
    ...
    private isUpdatingTask(): boolean {
        return (
            !!this.currentTaskToUpdate &&
            Object.keys(this.currentTaskToUpdate).length > 0
        );
    }
    ...
    async fetchType(type: string): Promise<void> {
        switch (type.toLowerCase()) {
            case this.types.assignee: {
                const res = await this.invoke<Response<Assignee[]>>(
                    "GET",
                    "sdk/assignees"
                );
            ...
            case this.types.issueType: {
                const res = await this.invoke<Response<IssueType[]>>(
                    "GET",
                    "sdk/issue-types"
                );
                if (res && !res.hasError && res.appData?.length > 0) {
            ...
            case this.types.issueStatus: {
                const res = await this.invoke<Response<IssueStatus[]>>(
                    "GET",
                    "sdk/statues"
                );
                if (res && !res.hasError && res.appData?.length > 0) {
    ...
    private configureForm(target?: HTMLElement): void {
        if (!this.form) return;
        if (target) {
    ...
    private async updateTask(): Promise<void> {
        ...
        const res: Response<Task> = await this.invoke(
            "PUT",
            `sdk/task/${formData.appData.id}`,
            formData
        );
    ...
    private addEventSelectOnChange(): void {
        ...
        issueTypeSelectElement.onchange = (e: Event) =>
            this.setCurrentSelectedFormFieldState(
                this.types.issueType,
                (e.target as HTMLSelectElement).selectedIndex
            );
        assigneeSelectElement.onchange = (e: Event) =>
            this.setCurrentSelectedFormFieldState(
                this.types.assignee,
                (e.target as HTMLSelectElement).selectedIndex
            );
        issueStatusSelectElement.onchange = (e: Event) =>
            this.setCurrentSelectedFormFieldState(
                this.types.issueStatus,
                (e.target as HTMLSelectElement).selectedIndex
            );
    }

    private setCurrentSelectedFormFieldState(
        field: string,
        index: number
    ): void {
        switch (field.toLowerCase()) {
            ...
            case this.types.issueStatus: {
                if (this.isUpdatingTask()) {
                    ...
                    this.currentTaskToUpdate!.taskStatus =
            ...
            case this.types.issueType: {
                if (this.isUpdatingTask()) {
                    ...
                    this.currentTaskToUpdate!.issueType =
    ...
    public collectFormData(): GenericRequest<Task> {
        this.toggleLoading(true);
        const taskRequest: GenericRequest<Task> = {
            appData: {
    ...
    private getCurrentSelectedIssueStatus(): IssueStatus {
        ...
        if (
            !this.currentSelectedIssueStatus ||
            Object.keys(this.currentSelectedIssueStatus).length <= 0
        ) {
    ...
    private getCurrentSelectedIssueType(): IssueType {
        ...
        if (
            !this.currentSelectedIssueType ||
            Object.keys(this.currentSelectedIssueType).length <= 0
        ) {
    ...
    private injectTaskDataToForm(task: Task) {
        ...
        const issueStatusSelectElement = document.getElementById(
            `status`
        ) as HTMLSelectElement;
        const assigneeSelectElement = document.getElementById(
            `assignee`
        ) as HTMLSelectElement;
        ...
        Array.from(issueStatusSelectElement.options).forEach(
            (option, index) => {
